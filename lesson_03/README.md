...
#### Типы кэшей в ZFS
**L2ARC** (Adaptive Replacement Cache) -- кэш-устройство, предназначенное для повышения производительности чтения в пуле. Этот кэш заполняется со временем и хранит данные на основе комбинации, какие блоки наиболее часто используются и используются в последнее время, что обеспечивает быстрый доступ к «горячим» данным.  
**ZIL** (ZFS Intent Log) -- лог-устройство, предназначенное для повышения производительности записи в пуле. Добавление SSD позволяет выполнять синхронную запись при высоких IOPS и очень низкой задержке, что в некоторых случаях повышает производительность приложений (например, базы данных и общие ресурсы NFS). 
